...
## 2026-09-17 follow-up #6 — BUG 3: Name/Email on the hold screen were free-typed and never persisted; BUG 4: Check-in's "Khách" fallback was a direct consequence

**Confirmed facts**: `bookings` has no name/email column at all (only `guest_note`); `profiles` has `display_name` but no email column of its own. `Reserve.jsx`'s `formName`/`formEmail` (web) and `ReserveView`'s `formName`/`formEmail` (iOS, pre-fix) were free-typed inputs `submitReserve()`/`AppState+Data.swift submitReserve()` never actually sent anywhere — `hold_seats()` is called with `p_note: null` only. iOS already pre-filled these fields from real identity (`goReserve()`: `formName = user?.displayName ?? ""`, `formEmail = userEmail ?? ""`) but still let them be freely edited and, like web, never sent the edited value anywhere either.

**BUG 3 fix**: both platforms now show Name read-only (from `profiles.display_name` via `s.user.name`/`user?.displayName`) once a real name exists, with a "Đổi trong Tài khoản"/"Change in Account" link to `goEditName()`. Email is always read-only (`s.user?.email`/`userEmail`), never editable — there is no write path for it in scope. When `display_name` is still empty (e.g. an OAuth sign-in whose provider supplied no name, or any account whose `raw_user_meta_data` never had a `display_name` key — `handle_new_user()`, migration 006 onward, only sets it from that key), a real input is shown instead, wired to a new `setNameAtHold()`/`AppState+Data.swift setNameAtHold(_:)` that calls the same `rename_display_name()` RPC Account's "Đổi tên" uses — a genuine write, not a value that goes nowhere — without navigating away (unlike a real rename, mid-hold-flow isn't the place to lose the guest's progress) and without the `name_change` notification (there's no prior organizer relationship yet to notify, and no "old name" to report). Hold stays disabled (`formOk = hasName`) until a real name exists. `src/screens/Confirmed.jsx`'s greeting (`s.formName.trim() || 'Bạn'`) also switched to `s.user?.name` — it was reading the same now-retired free-text field for an unrelated purpose.

**BUG 4 confirmed as a direct consequence, verified live** (not just inferred): built an isolated repro (`tests/e2e/setup.mjs` fixtures) — a fresh guest account's `profiles.display_name` starts as whatever `user_metadata.display_name` was at signup (via `handle_new_user()`), and calling the real `rename_display_name()` RPC (the exact call `setNameAtHold()`/Account's rename now both use) updates it correctly; the exact query `loadAttendanceGuests()` runs (`bookings.user_id` → `profiles.display_name`) then resolves to the real name, not the `'Khách'` fallback — confirmed with real query output: `["Trần Thị Mai"]`, not `["Khách"]`. The join itself was never the bug — Check-in's query was always correct — it just had nothing but empty/never-written names to resolve, for any account whose real name never made it into `profiles.display_name` in the first place (which, before this fix, included any guest who only ever "set" a name via the dead Reserve.jsx field).

**Could not get a live screenshot of Check-in itself for this verification**: `Dashboard.jsx`'s event list (`myEvents`) is filtered from a **static, bundled demo catalog** (`src/data/events.js`), not a live query of the `events` table — a dynamically-created test event (via `tests/e2e/setup.mjs`) never appears there regardless of real ownership (`myOrgEventKeys` only filters *which* catalog entries show, it doesn't add new ones), so "Điểm danh"/Check-in is unreachable via the UI for an ad hoc test event. This is a pre-existing, unrelated architecture constraint (not something touched by this pass) — flagging it here since it blocked a literal Attendance screenshot; the direct-query verification above is the closest available substitute and exercises the identical code path.

Both `vite build` and `xcodebuild` (iOS) succeed.
