...
## 2026-09-21 follow-up — Inbox star badge collision, settings-sheet overlay/animation/safe-area, read-row name de-emphasis

Three real-device bugs reported against 7210059's new Inbox controls, fixed on both platforms where applicable (Bug 2's root cause is iOS-only — see below).

**Bug 1 — star badge collided with the merged-avatar badge**: both badges were anchored to the same avatar corner (iOS: `.bottomTrailing` ZStack, `.offset(x:-2,y:2)`; web: `left:-2,bottom:-2` vs. the merged avatar's own `right:-2,bottom:-2` — the two only avoided full overlap by luck of opposite corners, not because either badge accounted for the other). Moved the star to the row's own far trailing edge instead, a sibling of the name/snippet column rather than living on the avatar at all — `src/screens/Inbox.jsx` (`InboxRow`, new `data-testid="inbox-row-star-badge"` span at the end of the row's flex children), `apps/ios/BanbeApp/Views/MessagingViews.swift` (`InboxRow.body`, `Image(systemName: "star.fill")` moved out of the avatar `ZStack` to a trailing sibling of the `VStack`+`Spacer`).

**Bug 2 — Inbox settings sheet obscured by the dock, janky entrance**: root cause is **iOS-only** — confirmed by reading `BottomTabBarOverlay.swift`'s own doc comment, which already states the bar's overlay window sits "above the app's own main window ... including any `.sheet()`/`UIPresentationController` content inside it, AT ANY DETENT." The settings sheet (and the feedback flow, same mechanism) are hand-rolled/`.fullScreenCover` content INSIDE the main window; `.inbox` staying in `BottomTabBar.visibleScreens` the whole time means the existing `updateVisibility(for:)` (screen-keyed) never hides the overlay for a same-screen presentation. Web has no equivalent bug: `Inbox.jsx`'s settings sheet already renders at `zIndex: 30`, above the dock's own `zIndex: 25` (`BottomTabBar.jsx`'s own comment already documents this ordering, established for `Notifications.jsx`'s action-sheet scrim) — confirmed by reading both z-index values, not assumed.
- **2a (dock hidden during the sheet)** — reused the EXACT `isHidden`-sync mechanism 4d549f9 established, rather than inventing a second one: `BottomTabBarOverlay.swift` gained `currentScreen`/`forcedHidden` stored state, a private `applyVisibility()` combining both (`forcedHidden || !visibleScreens.contains(currentScreen)`), and a new `setForcedHidden(_:)` alongside the existing `updateVisibility(for:)` (both now route through `applyVisibility()`). `MessagingViews.swift`'s `InboxView` calls `setForcedHidden(open || <other flag>)` from `.onChange(of: settingsOpen)`/`.onChange(of: feedbackOpen)`, plus a `.onDisappear` safety reset — covers the feedback flow's `.fullScreenCover` too, same root cause, essentially free once the mechanism exists.
- **2b (smooth entrance)** — the sheet's `if settingsOpen { settingsSheet }` toggle wasn't wrapped in `withAnimation` at all (an implicit, un-animated snap); new `openSettings()`/`closeSettings()` helpers wrap the state flip in `.spring(response: 0.3, dampingFraction: 0.8)` — the SAME parameters `AppState.noteScaffoldScroll` already uses for the tab bar's own scroll-collapse animation, reused rather than picked independently. The scrim keeps a plain `.opacity` transition; the sheet content itself now has `.transition(.move(edge: .bottom))` (was `.opacity` only, which reads as a fade/snap, not a slide).
- **2c (true full-height sheet)** — the sheet's rounded-top background is now `UnevenRoundedRectangle(topLeadingRadius: 18, topTrailingRadius: 18)` with `.ignoresSafeArea(edges: .bottom)` scoped to just that background layer, so the paper fill extends all the way through the home-indicator safe area with no exposed corner/gap below it, while the row content itself stays padded above the home indicator through ordinary (safe-area-respecting) layout — `MessagingViews.swift`'s `settingsSheet`.

**Bug 3 — read-row name/preview weight hierarchy**: both platforms previously dropped the READ row's name font-weight (700→600 web, `.bold`→`.semibold` iOS) as its ONLY de-emphasis signal, with no opacity/color change on the name itself — visually it just looked "slightly less bold," not clearly de-emphasized, and gave no headroom to de-emphasize further without risking dropping under the preview text's own weight. Fixed on both platforms: the name's font-weight is now FIXED (`700`/`.semibold`) across both unread and read states — "stays bold" — and de-emphasis is done entirely via opacity/color instead (web: `opacity: unread ? 1 : 0.62` on the name span, `display()`'s existing `ink` color; iOS: `.foregroundStyle(app.palette.ink.opacity(unread ? 1 : 0.6))`). Since the preview line's own weight tops out at `600`/`.semibold` (unread) or `400`/`.regular` (read), and the name is always `700`/`.semibold`-or-heavier, the name can never read as lighter-weight than its own row's preview text at either state — `src/screens/Inbox.jsx` (`InboxRow`), `apps/ios/BanbeApp/Views/MessagingViews.swift` (`InboxRow.body`).

`vite build` clean; iOS `xcodegen generate` + `xcodebuild -destination 'generic/platform=iOS Simulator'` BUILD SUCCEEDED. No schema/migration changes this pass. Verified by code inspection + both platforms' clean builds only — no live device/simulator run to visually confirm the slide animation or safe-area fill, flagging per this ticket's own "confirm" ask.
