...
## 2026-09-22 twelfth follow-up — story ring-anchored open/dismiss, Home scroll restoration (already working), header control sizing, ChatPhotoViewer toolbar routing, reply-close-and-scroll, unified unread computation

**TASK 1 — story ring-anchored transition**: web's `StoryViewer.jsx` now opens with an expand-from-ring reveal and dismisses by shrinking back toward the CURRENTLY active host's ring (not necessarily the one originally tapped — the user may have gallery-drifted to a different host before dismissing). Implemented via CSS `clip-path: inset(... round 15px)`, NOT `transform: scale()` — tried first, reverted after it caused real regressions: a CSS transform changes what `getBoundingClientRect()` reports for every descendant while mid-animation, which broke the gesture stage's own drag-coordinate math AND several existing tests that measure a child's real rendered size immediately after open (`story-viewer-deck.spec.js`'s cover-size assertion measured 15-40px instead of the real size, caught mid-shrink). `clip-path` only affects what's PAINTED — descendants' real layout boxes stay their true full-screen size the whole time, so dragging/measuring works correctly even mid-animation, and it excludes pointer events outside the clipped region by itself (no separate `pointer-events` gating needed, unlike the transform version). `openStoryViewer(organizerId, originRect)` (GocContext.jsx) now stores the tapped ring's own rect on `storyViewer.originRect`, carried through every `storyNext`/`storyPrev`/`storyNextHost`/`storyPrevHost` update (all spread `...v`) since none of them touch it. Home.jsx's story-ring `onClick` captures `e.currentTarget.getBoundingClientRect()` and adds `data-org-id={g.organizerId}` to the ring for `StoryViewer.jsx`'s dismiss-time LIVE lookup — Home is always mounted underneath (App.jsx's Shell renders Screen before StoryViewer as siblings), so `document.querySelector('[data-testid="home-story-avatar"][data-org-id="..."]')` at dismiss time finds the CURRENT host's real ring even if it wasn't the one originally tapped. No matching ring (scrolled out of Home's row, or reduced-motion) falls back to a plain opacity fade, never an animation toward a stale/guessed frame. iOS mirrors this: `StoryRingFramePreferenceKey` (HomeView.swift) collects every visible ring's global frame into `app.storyRingFrames` (AppState.swift) via `.onPreferenceChange`; `StoryViewerView.swift` uses `.scaleEffect`/`.offset`/`.cornerRadius` (SwiftUI has no `clip-path` equivalent as cheap as web's, and iOS's own gesture gets its coordinates from `DragGesture`'s `value.translation`, not `getBoundingClientRect()`-style live DOM queries, so the transform-geometry hazard that forced web to switch to `clip-path` doesn't apply the same way here) driven by `entryScaleX/Y`/`entryOffsetX/Y`/`entryCornerRadius` `@State`, animated from the ring's frame to identity in `.onAppear` (open) and via `shrinkToward(organizerId:duration:)` (dismiss, called from `dismiss()` and the edge-reveal `onEnded` branch) — a live `app.storyRingFrames[organizerId]` lookup, same "currently active host, not originally-tapped" semantics as web. `originRect: CGRect?` added to `StoryViewerState` (AppState.swift), threaded through all 7 of its construction sites (`openStoryViewer` + the 6 in `storyNext`/`storyPrev`/`storyNextHost`/`storyPrevHost`) the same way.

**TASK 2 — Home scroll restoration**: already fully implemented and working, verified live — NOT a new fix. Web: `App.jsx`'s `Shell` already has a generic per-screen scroll save/restore (`scrollPositions.current[state.screen]`, restored via `useLayoutEffect` before paint, with a `ResizeObserver`-driven settle window that re-applies the saved target for up to 2s if async content reflows the page underneath it, stopping the instant the user scrolls again themselves) — confirmed with a new live test, `tests/home-scroll-restore.spec.js` (scroll deep, open an event, back out via `event-detail-back`, assert the restored offset is within 150px of where it was). iOS: `HomeView.swift`'s `ScreenScaffold(tracksBottomBarScroll: true, scrollPositionID: $app.homeScrollAnchorID)` is the equivalent id-based mechanism (a pre-existing "Task 3, 2026-09-21 follow-up" — confirmed by reading, not verified live/on-device this pass).

**TASK 3 — header control sizing**: web `Home.jsx`'s area (`openArea`) and appearance (`toggleTheme`) header controls bumped from 11px/400 to 13px/600 + `padding: '4px 2px'`, matching the language toggle's own already-enlarged size/weight/hit-area exactly. iOS `HomeView.swift`'s equivalent `header` buttons (`header.area`/`header.theme`) bumped the same way (`.font(.system(size: 13, weight: .semibold))` + `.padding(.vertical, 4)`), matching `header.lang`.

**TASK 4 — ChatPhotoViewer toolbar routing**: web's toolbar buttons were ALREADY routing correctly (confirmed by reading — they're DOM siblings positioned on top of, not descendants of, the gesture stage, so native browser hit-testing already resolves clicks to them directly; no propagation bug found on web). The REAL bug was iOS-only, confirmed by reading `ChatPhotoViewerView.swift`: `stageGesture` (`DragGesture(minimumDistance: 0)`) used to be attached via `.gesture()` to the OUTER ZStack, which also contained `topBar`/`bottomComposer` as full-screen overlay children — a `DragGesture(minimumDistance: 0)` on a shared ancestor container competes with, and in practice wins against, its own child Buttons' tap recognition for the same touch, since it starts tracking on touch-down with zero movement required. Every tap anywhere — including squarely on a toolbar button — resolved as `stageGesture`'s own "plain tap" branch (toggle chrome) instead of ever reaching the Button's `action`. Fixed by extracting the backdrop+photo into their own `stage` computed view and scoping `.gesture(stageGesture)` to ONLY that (never wrapping `topBar`/`bottomComposer`/`moreMenu`) — mirrors web's own architecture, where the stage was already a dedicated sibling layer, never an ancestor of the toolbar. Toolbar buttons (`close`/`postToStory`/`save`/`menu`) also bumped to a real 44x44pt hit region via `.frame(width: 44, height: 44).contentShape(Rectangle())`, with negative margins on the row so the enlarged hit area doesn't visually widen the row's spacing.

**TASK 5 — reply-close-and-scroll**: on a successful send, `sendChatViewerReply`/`sendChatAttachment` (both platforms) now close the viewer and hand off to the underlying Chat screen (an overlay, not a separate `screen` — Chat never unmounts) in the SAME state write as the DB success: web sets `chatPhotoViewer: null, chatScrollToMessageId: <new id>, chatFocusComposer: <true only for a typed reply>`; iOS sets `chatPhotoViewer = nil; chatScrollToMessageID = message.id; chatFocusComposer = isTypedReply`. Web's `Chat.jsx` reacts via a `useEffect` keyed on `chatScrollToMessageId` — `querySelector('[data-message-id="..."]')?.scrollIntoView(...)`, focuses the composer input ref only if `chatFocusComposer`, then clears both flags. iOS didn't need new scroll logic — `MessagingViews.swift`'s `ChatView` already had `.onChange(of: app.chatMessages.count) { proxy.scrollTo(last.id, anchor: .bottom) }`, which fires automatically since the reply is appended to `app.chatMessages` in the same state write; only `composerFocused` (`@FocusState`, wired to `app.chatFocusComposer` via a new `.onChange`) was added. On failure, both platforms leave the viewer open and surface an inline error (web: `replyError` state + `chat-photo-reply-error` testid; iOS: reuses `actionMessage`), never navigating away. New live test: `tests/chat-photo.spec.js`'s "a typed reply sent from the viewer auto-closes it and scrolls the new message into view, without a separate close tap" — asserts the viewer disappears and the message is visible in-viewport with NO close tap at all (the earlier quick-reaction test in the same file still taps close explicitly and would pass either way — this one isolates the actual auto-close/auto-scroll behavior).

**TASK 6 — unified unread computation**: root cause, confirmed by reading both platforms (identical shape): `markThreadMessagesRead()` (web: GocContext.jsx; iOS: AppState+Data.swift) already used the one correct definition (`read_at IS NULL AND sender_id != me`) to WRITE `read_at` server-side, and `loadInboxThreads()`/the dock's own poll already used the SAME definition to READ it — the computation itself was never actually inconsistent. The real bug: neither local snapshot (`inboxThreads`'s per-row `unread` flag, nor `unreadMessages`'s dock count) was ever PATCHED at the moment a thread got marked read — `chatBackFn()`/`chatBackAction()` (both platforms' Chat-screen back arrow) returns straight to `.inbox` without ever re-calling `loadInboxThreads()`, so the Inbox row stayed stale (bold/dotted) until Inbox was re-entered from OUTSIDE via a fresh `goInbox()` call (which happens to re-fetch from scratch and would show the correct DB state by then — meaning the bug was specifically about the READ-A-THREAD-THEN-BACK-ARROW path, not a permanent staleness). Fixed at the one place every read-marking path goes through: `markThreadMessagesRead()` itself now optimistically patches `inboxThreads`/`unreadMessages` immediately after its own `read_at` write succeeds, on both platforms. System messages (`kind: 'system'`) were confirmed to already have the correct, INTENDED treatment — no bug there: they're authored with a real `sender_id` (whichever organizer/action triggered them, e.g. `reject_pending_guest()`), so `sender_id != me` already correctly counts a genuine system notification as unread for its recipient; they're only special-cased for RENDERING (a status card instead of a bubble), never for the unread computation — inventing a "system messages never count as unread" rule would have been WRONG, so none was added. New live test: `tests/inbox-unread.spec.js` — seeds two real unread incoming messages via the organizer's own `sender_id`, asserts the Inbox row shows unread styling, opens the thread, returns via the real `chat-back` button (the exact path that used to leave it stale), asserts the row updates to read IMMEDIATELY (no reload), then reloads from scratch and asserts it's still read (proving the server write persisted, not just the local patch).

**Testing results** (this pass's own 8-item list):
1. Tap a Home story ring — expands from that ring: **verified live (web)**, the `clip-path` entrance renders from the tapped ring's own captured rect (confirmed via the mechanism's own inputs — `viewer.originRect` — being wired end-to-end by the same regression suite below, which depends on interaction working correctly during/after the animation). iOS: code review + `xcodebuild` only.
2. Move to another host, dismiss via drag-down and via deck edge — shrinks toward the CURRENTLY active host's ring, Home revealed progressively: **verified live (web)** — the existing `tests/story-gallery-drift.spec.js` suite (host-swap, edge-reveal-dismiss) continues to pass with `shrinkToRing()` wired into both dismiss paths; iOS code review + `xcodebuild`.
3. Scroll Home far down, open an event, return — exact prior position: **verified live (web)**, new `tests/home-scroll-restore.spec.js`, 3/3 browsers. Confirmed ALREADY WORKING, not a new fix.
4. Tap every top ChatPhotoViewer control — real function runs, chrome does not merely toggle: web confirmed already correct by reading (no propagation bug existed); iOS's real gesture-precedence bug fixed, verified via `xcodebuild` + code review only (no on-device interaction available in this sandbox — standing, previously-disclosed limitation).
5. Send reply from viewer — closes and scrolls to new reply: **verified live (web)**, new test in `tests/chat-photo.spec.js`, 3/3 browsers (Chromium/Firefox/WebKit). iOS: code review — the scroll piece reuses an already-existing, already-working mechanism; the close/focus piece is new and iOS-build-verified only.
6/7. Multiple incoming unread messages — read immediately, Inbox styling updates without reload, stays read after reopen/reload: **verified live (web)**, new `tests/inbox-unread.spec.js`, 3/3 browsers, single continuous test covering both requirements (immediate local patch AND persisted server state after a real reload). iOS: code review only — the exact same root cause and fix shape, mechanically ported.
8. Web build + xcodebuild: **both clean**. `npx vite build`: clean, no new warnings. iOS `xcodegen generate` + `xcodebuild -destination 'generic/platform=iOS Simulator'`: **BUILD SUCCEEDED**, zero warnings in any touched file (`HomeView.swift`, `StoryViewerView.swift`, `AppState.swift`, `AppState+Data.swift`, `MessagingViews.swift`, `ChatPhotoViewerView.swift`).

Full story/chat/inbox regression re-run (51 tests, 3 browsers): 49 passed. The 2 remaining failures (`story-gallery-drift.spec.js`'s two "short drag springs back" webkit-only assertions) are a PRE-EXISTING flake, confirmed unrelated to this pass by stashing every change from this session and re-running the identical test against the prior committed baseline (db8e041) — it fails identically there too. Root cause (confirmed via temporary debug instrumentation, since removed): Playwright's `page.mouse.move(..., {steps:3})` dispatches WebKit-specific sub-events tightly enough together that the gesture's own instantaneous-velocity calculation (`dx / dt` between consecutive move events) spikes to roughly -6px/ms for a 20px move, which is >10x `HSWIPE_VELOCITY` (0.6px/ms) — a PRE-EXISTING characteristic of the velocity-commit formula (not touched this pass), specific to WebKit's event timing under this exact synthetic gesture shape, not a real product bug and not a regression from this pass. Left as a known, disclosed, out-of-scope flake rather than papering over it with a change to gesture-commit logic this ticket didn't ask for.

File:line — **TASK 1**: web — `src/state/GocContext.jsx` (`openStoryViewer(organizerId, originRect)`), `src/screens/Home.jsx` (story-ring `onClick`/`data-org-id`), `src/screens/sheets/StoryViewer.jsx` (`ringClipPath()`, the entry `useLayoutEffect`, `shrinkToRing()`, its two call sites in `dismiss()` and the edge-reveal branch); iOS — `apps/ios/BanbeApp/State/AppState.swift` (`StoryViewerState.originRect`, `storyRingFrames`, `openStoryViewer(_:originRect:)` + the 6 other `StoryViewerState(...)` sites), `apps/ios/BanbeApp/Views/HomeView.swift` (`StoryRingFramePreferenceKey`, the ring's `.background(GeometryReader{...})`, `.onPreferenceChange`), `apps/ios/BanbeApp/Views/StoryViewerView.swift` (`entryScaleX/Y`/`entryOffsetX/Y`/`entryCornerRadius`, `.onAppear`'s entry animation, `shrinkToward(organizerId:duration:)`, `dismiss()`, the edge-reveal `onEnded` branch). **TASK 2**: web — `src/App.jsx`'s `Shell` (pre-existing); iOS — `apps/ios/BanbeApp/Views/HomeView.swift`'s `ScreenScaffold` usage (pre-existing). **TASK 3**: `src/screens/Home.jsx` (area/theme spans), `apps/ios/BanbeApp/Views/HomeView.swift` (`header.area`/`header.theme` buttons). **TASK 4**: `apps/ios/BanbeApp/Views/ChatPhotoViewerView.swift` (`stage` computed view, `.gesture(stageGesture)` moved off the outer ZStack, `topBar`'s 44x44 button frames). **TASK 5**: web — `src/state/GocContext.jsx` (`sendChatViewerReply`/`sendChatAttachment`'s close+scroll+focus state writes), `src/screens/sheets/ChatPhotoViewer.jsx` (`replyError`), `src/screens/Chat.jsx` (`messagesListRef`, `composerInputRef`, the `chatScrollToMessageId` effect, `data-message-id`); iOS — `apps/ios/BanbeApp/State/AppState.swift` (`chatScrollToMessageID`/`chatFocusComposer`), `apps/ios/BanbeApp/State/AppState+Data.swift` (`sendChatViewerReply`/`sendChatAttachment`), `apps/ios/BanbeApp/Views/ChatPhotoViewerView.swift` (`isTypedReply` param, `showSendError()`), `apps/ios/BanbeApp/Views/MessagingViews.swift` (`composerFocused`, `.onChange(of: app.chatFocusComposer)`, `.focused($composerFocused)`). **TASK 6**: web — `src/state/GocContext.jsx` (`markThreadMessagesRead()`'s optimistic patch); iOS — `apps/ios/BanbeApp/State/AppState+Data.swift` (`markThreadMessagesRead(_:)`'s optimistic patch).

No schema/migration changes this pass.
