...
## 2026-09-22 second update — a StoryViewer-specific overlay-retention pattern, in case this file's own viewers ever need it

Not a change to `PhotoViewerView`/`ChatPhotoViewerView` (neither was touched this pass), but a pattern worth knowing about if either ever needs an interactive edge-swipe-back reveal of its own: StoryViewer's fullscreen overlay used to be nulled out (`storyViewer = nil`/`storyViewer: null`) the moment Event Detail opened from a story, which meant an interactive back-gesture had no live view left to reveal underneath — only a reconstruction on a COMPLETED back action. Fixed by keeping the SAME overlay instance retained (paused via a new `isSuspended` flag, dropped below the covering screen in z-order/hit-testing at rest, raised back to full visibility the instant a peek starts) rather than tearing it down and rebuilding it. Full root cause, fix, and file:line: `07-notifications.md`'s "2026-09-22 tenth follow-up" entry (BUG 1). If `PhotoViewerView`/`ChatPhotoViewerView` ever grow a "reachable from a deeper screen, needs an interactive back-reveal" requirement, this is the established pattern to reuse rather than reinventing a second one.

## 2026-09-22 third update — the REAL root cause of "swipe to a new host, old cover stays" was a shared image-cache bug, plus a preload/cache lifecycle convention worth reusing

Two things from this pass's own follow-up worth recording here since both are general patterns this file's own viewers could hit:

1. **`RemoteImage`'s stale-`@State` bug (`Components.swift`)** — a real, narrow bug, not "SwiftUI reuses view identity" in the abstract (that reuse is correct and expected). `@State private var image: UIImage?` is only ever seeded from `init()`'s own `State(initialValue:)` the FIRST time a view is created at a given tree position; a caller that changes `path` while the SAME `RemoteImage` instance stays mounted (StoryViewer's `EventShareCard` swiping between hosts; potentially any other screen that swaps a `CatalogPhoto`'s `path` in place) does NOT get that fresh seed again. `.task(id: path)` DOES restart correctly on a path change, but its OLD guard (`guard image == nil else { return }`) couldn't distinguish "no image loaded yet" from "the WRONG image is already sitting there" — so it silently kept showing the stale photo forever. Fixed by checking the synchronous cache for the NEW path specifically, and otherwise clearing the stale image before awaiting a fresh load. `PhotoViewerView`/`ChatPhotoViewerView` don't use `CatalogPhoto`/`RemoteImage` (they go through `AsyncImage` directly, which has its own separate quirks — see this file's earlier 2026-09-xx entries), so neither was affected by this SPECIFIC bug, but the general lesson — a cache `@State` that only checks "do I have something" instead of "do I have the CURRENT thing" — is worth remembering if either viewer ever grows its own local image cache.
2. **Background preload + cache lifecycle convention** — StoryViewer now preloads the current host's full story set plus adjacent hosts' first story/cover the moment it opens, using `.task(id: <the thing that changes when the target set changes>)` for automatic cancellation (switching hosts or the view disappearing both cancel any in-flight preload for free, no manual bookkeeping) and the EXISTING image cache (`PhotoLoader`/`URLCache.shared`) rather than inventing a second one — so a later real render just paints from what's already warm. If `PhotoViewerView`'s own gallery ever wants adjacent-photo preloading (paging through a real photo gallery, not just a single image), this is the pattern to reuse: `.task(id:)` for lifecycle, the shared loader for storage, never a bespoke cache. Full root cause, fix, and file:line for both: `07-notifications.md`'s "2026-09-22 eleventh follow-up" entry.

## 2026-09-22 fourth update — origin-rect shrink-back convention extended to an OPEN-side animation too, and why it had to use `clip-path` on web instead of `transform: scale`

`PhotoViewer.jsx`'s own established convention (`originRect`, computed dx/dy/scale against the live current rect, animated via `transform`) only ever drove the DISMISS side — opening always used a generic `gocIn` fade/scale, never a literal expand-from-thumbnail. StoryViewer's own ring-anchored open (07-notifications.md's "2026-09-22 twelfth follow-up", Task 1) needed a REAL expand-from-origin on open too, which surfaced a real hazard worth recording here in case `PhotoViewer.jsx`/`ChatPhotoViewer.jsx` ever want the same: a `transform: scale()`-driven open animation changes what `getBoundingClientRect()` reports for every descendant for as long as it's mid-flight, which broke both the gesture stage's own drag-coordinate math (a drag starting during the ~260ms open animation computed absolute page coordinates against a still-shrinking/offset box) and several PRE-EXISTING tests that read a child's real rendered size immediately after open. Neither `PhotoViewer.jsx` nor `ChatPhotoViewer.jsx` currently animate ANYTHING on open this way (both still use `PhotoViewer.jsx`'s existing dismiss-only convention), so neither was touched — but if either ever grows a real expand-from-thumbnail open animation, use `clip-path: inset(...)` (animating from the origin rect's own inset to `inset(0px 0px 0px 0px)`) instead of a transform — `clip-path` only affects what's PAINTED, so every descendant's real layout box/rect stays correct the entire time, and it naturally excludes pointer events outside the clipped region on its own (no separate `pointer-events` gating needed, which the transform version required and which the clip-path version doesn't). Full root cause (including the specific regressions this caused before being caught and fixed) and file:line: `07-notifications.md`'s "2026-09-22 twelfth follow-up" entry, Task 1.
