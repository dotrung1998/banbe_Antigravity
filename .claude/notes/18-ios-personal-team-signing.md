...
**2026-10-02 addendum — distinct Home Screen icon/name**: `PersonalTeamDebug` installs as **"Banbe Dev"**; the normal Debug/Release build is still **"banbe"** (its existing, unchanged display name — untouched by this addendum). Both can be installed on the same iPhone at once, as two entirely separate apps (different bundle ids — `com.banbe.ios.dev` vs `com.banbe.ios` — means separate Home Screen entries, separate sandboxed storage, separate everything). Stopping Xcode does **not** uninstall "Banbe Dev" — it stays on the device exactly like any other installed app until you delete it yourself; if you tap the OLD "banbe" icon after a `PersonalTeamDebug` session, you're opening whatever was last built/signed for over there under whichever config that was, not "reverting" anything.
...
- `apps/ios/project.yml` (2026-10-02 addendum) — `CFBundleDisplayName` changed from the literal string `banbe` to `$(APP_DISPLAY_NAME)` (a build-setting variable Xcode substitutes into Info.plist at build time — the same mechanism `$(PRODUCT_BUNDLE_IDENTIFIER)` already relies on for `CFBundleIdentifier`). `APP_DISPLAY_NAME` is defined as `banbe` in the target's base settings (Debug/Release: unchanged from before) and overridden to `Banbe Dev` only under `settings.configs.PersonalTeamDebug`. Verified in the actual built `.app` bundle's `Info.plist`, not just `-showBuildSettings`.
