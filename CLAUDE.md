...
- [.claude/notes/16-refund-lifecycle.md](.claude/notes/16-refund-lifecycle.md) — host cancel → refund destinations → batch/mark-sent → dispute: WORKING (web + iOS); migration chain (069-078) and several real root-cause bugs documented — READ FIRST before touching refunds again
- [.claude/notes/17-ux-foundation-release.md](.claude/notes/17-ux-foundation-release.md) — Action Center, held-booking-vs-ticket gating, organizer create-event FAB, shareable profile card + universal link, Banbe Pulse ranked story: WORKING (web + iOS); migrations 079-080; read before touching any of these five again — especially why Pulse is NOT a `stories` table row
- [.claude/notes/18-ios-personal-team-signing.md](.claude/notes/18-ios-personal-team-signing.md) — `PersonalTeamDebug` iOS build config/scheme for local free-Apple-ID real-device testing (no Push, no Associated Domains): WORKING; read before touching iOS signing/entitlements again
