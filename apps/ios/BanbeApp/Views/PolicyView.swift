...
/// Port of src/screens/Policy.jsx — banbe_User_Policy.md v1.0, rendered in
...
struct PolicyView: View {
    ...
    var body: some View {
        ScreenScaffold {
            VStack(alignment: .leading, spacing: 0) {
                    ...
                    Text("Điều khoản và quyền riêng tư · v1.0 · Terms and privacy")
    ...
    private var summaryList: some View {
        VStack(alignment: .leading, spacing: 10) {
            summaryItem("banbe là trung gian kết nối bạn với người tổ chức (host). Host tạo sự kiện; bạn giữ chỗ trên banbe. banbe không xử lý thanh toán: nếu sự kiện có thu tiền, việc trả tiền là chuyện riêng giữa bạn và host, diễn ra ngoài banbe.",
                        "banbe is an intermediary connecting you with hosts. Hosts run the events; you hold a seat on banbe. banbe does not handle payments: if an event charges, paying is arranged directly between you and the host, outside banbe.")
            summaryItem("Vì banbe không nhận tiền và không có ghi chép nào về khoản bạn đã trả, chúng tôi không thể hoàn tiền hay đòi tiền giúp bạn. Giá và điều kiện hoàn tiền do host công bố trên trang sự kiện; hãy đọc trước khi trả tiền cho ai.",
                        "Because banbe receives no money and holds no record of what you paid, we cannot refund you or recover money for you. Price and refund terms are published by the host on the event page; read them before you pay anyone.")
            ...
            summaryItem("Giữ chỗ là bạn nhận được email kèm vé ghi mã đặt chỗ sáu ký tự; xuất trình vé này tại cửa. Nếu không đi được, hãy huỷ để host mở lại chỗ cho người khác.",
                        "Holding a seat sends you an email with a ticket showing your six-character booking code; show that at the door. If you cannot come, cancel so the host can release the seat.")
            summaryItem("Chúng tôi ghi nhận số điện thoại, email, tên, các lần đặt chỗ và tin nhắn của bạn với host. Host thấy tên, mã đặt chỗ và số điện thoại của bạn.",
                        "We store your phone number, email, name, bookings and your chats with hosts. Hosts see your name, booking code and phone number.")
            ...
            summaryItem("Bạn xem, sửa và xoá tài khoản bất kỳ lúc nào trong Tuỳ chọn; muốn lấy bản sao dữ liệu thì gửi yêu cầu theo Điều B8. Tài khoản không hoạt động liên tục 6 tháng, hoặc 14 tháng nếu bạn có hồ sơ tổ chức, sẽ được xoá tự động sau các email thông báo.",
                        "You can view, correct and delete your account at any time in Preferences; for a copy of your data, request it under Section B8. An account left inactive for 6 months, or 14 months if you have an organizer profile, is deleted automatically after email notices.")
            summaryItem("Chúng tôi không bán dữ liệu và không chạy quảng cáo. Tin nhắn từ banbe chỉ gồm mã OTP đăng nhập và thông báo về sự kiện bạn đã đặt hoặc tổ chức. Mọi tin nhắn khác tự xưng là banbe, nhất là tin xin tiền hoặc thông tin tài khoản, đều là giả mạo.",
                        "We do not sell data and do not run ads. A text from banbe only ever carries a sign-in OTP or a notice about an event you booked or are hosting. Any other message claiming to be banbe, especially one asking for money or account details, is fake.")
        }
    }
    ...
    private var partA: some View {
        Group {
            ...
            section("A4", "A4. Giữ chỗ, và chuyện tiền nong", "A4. Holding a seat, and money") {
                bi("Bấm “Giữ chỗ” tạo một đặt chỗ với mã sáu ký tự và xác nhận ngay. Bạn nhận email kèm vé ghi mã đặt chỗ; bạn xuất trình vé này tại cửa. Mỗi khách một đặt chỗ đang hiệu lực cho mỗi sự kiện. Số chỗ do host đặt ra; hết chỗ thì không giữ thêm được.",
            ...
            section("A5", "A5. Huỷ và có mặt", "A5. Cancelling and showing up") {
                bi("Bạn huỷ chỗ bất kỳ lúc nào trước giờ sự kiện, ngay trong ứng dụng. Host có thể huỷ sự kiện; chúng tôi báo cho những người đang giữ chỗ. Nếu bạn đã trả tiền cho host, việc hoàn tiền theo đúng điều kiện host đã công bố và được giải quyết trực tiếp giữa bạn và host: banbe không giữ tiền, không có ghi chép về khoản đã trả, nên không hoàn tiền thay host và không theo dõi được việc hoàn tiền. Quyền đòi host theo pháp luật của bạn không đổi, và bạn có thể báo cho chúng tôi qua [contact email] để chúng tôi xem xét hồ sơ host.",
                ...
                bi("Tại cửa, host ghi nhận bạn đã đến theo mã trên vé, hoặc theo tên nếu bạn không mở được vé. Không đến mà không huỷ có thể bị ghi vắng mặt; số lần tham dự và vắng mặt hiện cho host của các sự kiện bạn đặt sau, và vắng mặt nhiều lần có thể giới hạn số chỗ bạn được giữ.",
            ...
            section("A7", "A7. Cư xử và an toàn", "A7. Conduct and safety") {
                bi("Không quấy rối, đe doạ, lừa đảo, gửi tin rác hay chia sẻ thông tin cá nhân của người khác. Tin đã gửi không sửa hay xoá được. Nhân viên banbe chỉ đọc trò chuyện khi có báo cáo, tranh chấp hoặc yêu cầu của pháp luật. Chúng tôi không kiểm tra giấy tờ tuỳ thân, giấy phép hay lý lịch của host và khách, và không có mặt tại sự kiện: hãy đọc hồ sơ host, báo cho bạn bè biết bạn đi đâu, rời đi nếu thấy không an toàn, và báo cho chúng tôi qua [contact email]; khẩn cấp gọi 113 hoặc 115.",
    ...
    private var partB: some View {
        Group {
            ...
            section("B3", "B3. Mục đích và cơ sở xử lý", "B3. Purposes and legal basis") {
                ...
                bi("Chúng tôi không dùng dữ liệu cho quảng cáo, không phân tích nội dung trò chuyện, và không ra quyết định tự động ảnh hưởng đến bạn ngoài việc đếm số lần vắng mặt như nêu tại Điều A5.",
            ...
            section("B4", "B4. Ai nhìn thấy gì", "B4. Who sees what") {
                policyTable(
                    ...
                    rows: [
                        ["Số điện thoại, email\nPhone number, email", "Bạn, banbe, và host của sự kiện bạn đặt. Email chỉ bạn và banbe thấy.\nYou, banbe, and the host of an event you book. Email stays between you and banbe."],
            ...
            section("B5", "B5. Bên xử lý thay chúng tôi", "B5. Who processes data for us") {
                bi("Chúng tôi dùng: Supabase (cơ sở dữ liệu, xác thực, lưu trữ tệp); Vercel (lưu trữ ứng dụng web); [Vietnamese SMS provider] để gửi mã OTP; Google để gửi email đăng nhập, email vé và email thông báo. Các bên này chỉ xử lý theo chỉ dẫn của chúng tôi và không được dùng dữ liệu cho mục đích riêng.",
            ...
            section("B7", "B7. Lưu bao lâu, và khi bạn xoá tài khoản", "B7. How long we keep it, and what deletion does") {
                ...
                bi("Xoá tài khoản là một lần bấm trong Tuỳ chọn, không cần gửi yêu cầu. Việc xoá bị từ chối trong khi bạn còn sở hữu sự kiện đang mở; hãy huỷ trước. Việc xoá tự động do không hoạt động cũng theo ngoại lệ này: tài khoản được giữ lại cho đến khi sự kiện kết thúc hoặc bị huỷ, và các email thông báo nêu tại Điều A3 được gửi tới địa chỉ bạn đã đăng ký. Sau khi xoá, chúng tôi không thể khôi phục tài khoản.",
            ...
            section("B10", "B10. Bảo mật và sự cố", "B10. Security and incidents") {
                bi("Dữ liệu được mã hoá khi truyền. Quyền truy cập được kiểm soát theo từng dòng ngay trong cơ sở dữ liệu, nên host chỉ thấy khách của mình và khách chỉ thấy đặt chỗ của mình. Số nhân viên có quyền truy cập được giữ ở mức tối thiểu và mọi truy cập được ghi lại.",
            ...
            section("B11", "B11. Lưu trữ trên thiết bị", "B11. On-device storage") {
                bi("banbe lưu phiên đăng nhập và tuỳ chọn ngôn ngữ trên thiết bị của bạn để bạn không phải đăng nhập lại mỗi lần. Chúng tôi không dùng cookie quảng cáo hay công cụ theo dõi của bên thứ ba.",
