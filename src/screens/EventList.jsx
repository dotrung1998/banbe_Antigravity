...
// Opened from the "Going"/"Saved" cards on Account — a plain list of the
// matching events, in its own view rather than redirecting to Home, so
// "back" is a single step to Account instead of losing the trip there.
export default function EventList() {
  const { state, T, trStatus, stripKm, goEvent, backFromEventList } = useGoc();
  const s = state;

  const isGoingMode = s.eventListMode === 'going';
  const keys = isGoingMode ? s.attending : (s.favorites || []);
  const list = keys.map(k => EVENTS.find(e => e.key === k)).filter(Boolean);

  const title = isGoingMode ? T('Đang tham gia', 'Going') : T('Đã lưu', 'Saved');
  const emptyMsg = isGoingMode
    ? T('Bạn chưa tham gia sự kiện nào.', "You're not going to any events yet.")
    : T('Bạn chưa lưu sự kiện nào.', "You haven't saved any events yet.");

  return (
    <div style={{ animation: 'gocIn 0.32s cubic-bezier(.22,.61,.36,1) both', minHeight: '100%', background: paper }} data-screen-label={isGoingMode ? 'Going' : 'Saved'}>
